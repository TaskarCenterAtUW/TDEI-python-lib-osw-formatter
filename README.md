...
| `max_geometry_vertices` | `2000` | Maximum coordinate vertices per line or polygon feature. Applied to OSW input and to generated OSW, and it sets the point at which OSW to OSM conversion splits a long run of coordinates into several joined ways. |
...
`max_geometry_vertices` caps how many coordinate vertices a single line or polygon feature may carry. It applies to `edges`, `lines`, `polygons` and `zones`; point and node datasets are unaffected. Polygon vertices are counted across the exterior and all interior rings, and a ring's repeated closing coordinate is not counted twice.

A feature over the limit is reported with the validator's message, whether it arrives as OSW input or is produced by OSM → OSW conversion:

```
Feature 'e1' in 'edges.geojson' contains 2001 geometry vertices. Maximum allowed is 2000.
```

The same setting decides where OSW → OSM conversion breaks up a long run of coordinates. An OSM way cannot hold more nodes than this, so a longer run becomes several ways that share a node and stay joined. That happens even for input the validator accepts, because the two count differently: the validator counts unique vertices and ignores a ring's closing coordinate, while an OSM way counts every node reference. A ring of exactly 2000 unique vertices is therefore valid, yet needs 2001 references and cannot be a single way.
